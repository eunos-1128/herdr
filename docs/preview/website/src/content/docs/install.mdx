...
This installs Herdr into the global pixi environment. Update it with `pixi global update herdr` when you want to refresh the installed version.
...
`herdr update` is for installs managed by Herdr's own installer. Homebrew, mise, pixi, and Nix installs are updated through those package managers instead.
